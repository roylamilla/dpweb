        ...
        if (json.status) {
            ...
            json.data.forEach(usuario => {
                ...
                nueva_fila.innerHTML = `
                            ...
                            <td>${estado}</td>
                            <td>
                                <a href="`+ base_url + `edit-cliente/` + usuario.id + `">Editar</a>
...
if (document.querySelector('#frm_edit_cliente')) {
    // evita que se envie el formulario
    let frm_cliente = document.querySelector('#frm_edit_cliente');
...
async function actualizarCliente() {
    ...
    let respuesta = await fetch(base_url + 'control/UsuarioController.php?tipo=actualizar_cliente', {
        method: 'POST',
        mode: 'cors',
        cache: 'no-cache',
        body: datos
...
async function eliminar(id) {
    ...
    if (!json.status) {
        ...
        alert(json.msg);
        location.replace(base_url + 'cliente');
    }
}